...
| OpenAI APIキー | 質問・会話・画像説明・下書き・リライト | ✅（既定） |
| Anthropic APIキー | 採点・テーマ等の裏方分析 | ✅ |
...
| `OPENAI_API_KEY` | OpenAIのAPIキー。ChatGPTのログイン情報ではなくAPIキーを設定 |
...
0. **見ている数字はいつのものか** — 最終計測日、未取得の件数、計測タイミングのばらつき、枠切れの有無
...
`spend-cap-reached`（403）は、開発者ポータルで設定した**月次の利用上限**に達した状態です。
月が変わるまで、またはポータルで上限を上げるまで復帰しません。

到達を検知すると `X_SPEND_CAP_UNTIL` に復帰見込み日を記録し、それまでは
メトリクス取得・手動投稿の取り込み・フォロワー記録を**投げずに見送ります**
（通らないリクエストで枠をさらに削らないため）。月が変わると自動で解除されます。

枠を使い切らないよう、読み取りは次のように絞っています。

- **数字が確定した投稿は二度と読み直さない。** 以前は投稿済みの行を毎回すべて読み直していたので、
  取り込んだ過去の手動投稿を含めて履歴が増えるほど毎週の消費が増え続けていました
- **判定は「投稿の古さ」ではなく「記録されている数字が投稿から何時間後のものか」で行う。**
  枠切れで計測が飛ぶと、投稿3日目の数字を持ったまま投稿が35日目になります。投稿の古さで
  判定するとこれを確定済みと誤認するので、計測時点の経過時間で見て自動的に取り直します
- **手動投稿の取り込みは `since_id` で前回以降だけを見る**（週次の自動実行のみ。
  `backfillManualPosts` は従来どおり遡ります）
...
**1セッション最大1回**補足を聞きます。次問も、既に答えた内容や否定した前提を
繰り返さないよう調整します。「どういう意味？」で現問の言い換え、「スキップ」
または「次の質問」で先へ進めます。補足を飛ばしても元回答は残ります。

質問生成には本人の会話に関するメモを反映します。短答/長答だけで質問の良否は決めません。
材料がない回答からは無理に投稿を作らず `no_material` として記録し、自動再生成対象から外します。
追問の保存列追加のため、反映時に **`Sheets.gs` の `setupSpreadsheet`** を実行してください。
Slack受信を更新するため、B（Slack）デプロイの更新も必要です。

追問は `INTERVIEW_FOLLOWUP_ENABLED=false` で停止できます。会話中の応答には最大1回の
生成API呼び出しが加わるため、実応答時間・費用は反映後に確認してください。
同時返信で処理中の場合は未記録と通知します。次の質問が届いてから再送してください。

新規内部ヘルパーはすべて `Interview.gs`（`gas/src/Interview.js`）に入ります。
`buildInterviewMemoryPrompt` / `validInterviewQuestion` / `hasPendingFollowup` /
`interviewAnswerText` / `planInterviewTurn` / `handleInterviewReplyLocked` は手動実行不要です。
設計議論、検証範囲、評価方法は [質問品質の設計記録](../docs/interview-quality.md) を参照してください。
...
| `METRICS_MIN_AGE_H` | `48` | 分析に使う最低の計測経過時間。これより早く計測した数字は、内容の差と計測の早さを区別できないため分析から外す |
| `METRICS_MAX_PER_RUN` | `150` | 1回の実行で読むツイート数の上限。X APIの枠を一度に食い潰さないための保険 |
