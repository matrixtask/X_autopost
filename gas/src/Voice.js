...
function buildStylePrompt(category) {
  ...
  var lines = [
    ...
    '- 挨拶や「〜について話します」という不要な前置きは省く。ただし初見で必要な主語・対象・状況は冒頭に自然に置く。何の話か分からないまま結論だけで始めない',
    '- 文脈を補う文も本人の口調にする。今回の回答原文の語尾・語彙・文の長短・間・確信の強さを優先し、Voiceの本人サンプルで補う。質問文の敬語や編集者の説明口調をそのまま移さない',
    '- 補足だけを評論調・広報調にせず、元の発言につながる一文や節としてなじませる。原文にない口癖や「〜なんだよね」の連発で本人らしさを演出しない。文体サンプルの出来事は事実の補完に使わない',
    '- 本人の確信の強さを保つ。迷いや保留を勝手に断定へ変えない。専門外の読者に必要な一言の説明は残す',
    '- 本人の回答にある場面・工夫・判断を残す。特に意外な比喩・捨てた選択肢・代償・留保を、誰でも言える教訓に薄めない。数字や固有名詞を必須にせず、足りない事実は補わない',
    ...
    '',
    'カテゴリごとに振り切る（いちばん大事）:',
    '- 笑いにAIが教訓を後付けしない。ただし本人が趣味と仕事を意外な比喩でつないだ場合、その面白さは切り離さず残す',
    '- neta（ネタ）: 笑わせにいく。オチで終える。教訓・学び・まとめに落とさない。短く切る',
    '- news（時事）: 本人の経験につながる部分を語る。接点がなければ使わない。断定を強制しない',
    '- evergreen（定番）: 本人の経験や判断を、読者の暮らし・仕事の言葉で伝える',
    '',
  ];
  lines.push(editorialFocusPrompt());
  ...
  if (samples.length) {
    ...
    lines.push('（文体サンプル未登録。今回の本人回答原文を口調の手本にし、一般的な話し言葉や架空の口癖で置き換えない。Voiceシートへの本人サンプル登録を推奨）');
  }
  return lines.join('\n');
}
